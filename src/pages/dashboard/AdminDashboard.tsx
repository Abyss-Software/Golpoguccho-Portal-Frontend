...
import FinancialRecordsTable from '@/components/finance/RecordsTable';
...
const AdminDashboard = () => {
  ...
  const [bestSellersStats, setBestSellersStats] = useState<any[] | null>(null);
  ...
  const handleCountsBetween = async () => {
    fetchCountsBetweenMutation.mutate(
      { start: startDate!, end: endDate! },
      {
        onSuccess: (res) => {
          setStats(res.body);
          setBestSellersStats(res.body.topSellingPackages);
        },
      }
    );
  ...
  return (
    <div>
      ...
      <hr className="m-20" />
      {userInfo?.role === UserRoles.ADMIN && <FinancialRecordsTable />}
    </div>
  );
};
